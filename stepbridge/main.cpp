...
private:
	...
	int HysteresisTrackIndex(int32_t filteredYKnob)
	{
		if (pattern.numTracks != lastNumTracksForHysteresis_)
		{
			...
			if (lastAcceptedTrackIndex_ >= pattern.numTracks) lastAcceptedTrackIndex_ = pattern.numTracks - 1;
		...
		const int32_t curEnd = (lastAcceptedTrackIndex_ + 1) * bandWidth + margin;
	...
	{
		int32_t result = 2048 - cv; // invert: cv = 2048 - adc, so 2048 - cv = adc
	...
	int HysteresisStepIndex(int32_t filteredXKnob, uint8_t length)
	{
		if ((int)length != lastLengthForHysteresis_)
		{
			...
			if (lastAcceptedStepIndex_ >= length) lastAcceptedStepIndex_ = length - 1;
			return lastAcceptedStepIndex_;
		}

		const int32_t bandWidth = 4096 / (int32_t)length;
		const int32_t margin = bandWidth / 8;
		const int32_t curStart = lastAcceptedStepIndex_ * bandWidth - margin;
		const int32_t curEnd = (lastAcceptedStepIndex_ + 1) * bandWidth + margin;
	...
	void ReadPanel()
	{
		...
		filteredYKnob_ += (yKnob - filteredYKnob_) >> 9;
		filteredXKnob_ += (xKnob - filteredXKnob_) >> 9;
		...
		if (page == Switch::Down)
		{
			...
			const int32_t trackSource = (pattern.cvTrackRoute > 0)
				? ApplyCvCal(CvToKnob(CVVal(pattern.cvTrackRoute - 1)),
		...
		{
			// Track no longer selected by Y on Middle page - stays as whatever
			// was last chosen on Down page. Moving Y while scrubbing steps caused
			...
			// units per 0-127 scale purely from X movement, crossing multiple
			// track-band boundaries with 6+ tracks). Fixing this by removing the
			// conflict entirely: Y now edits gate length on the scrubbed step,
			// and track selection is Down-page-only. lastAcceptedTrackIndex_ was
			// last written by HysteresisTrackIndex on Down page and carries over.
			...
			const int32_t xSource = (pattern.cvStepRoute > 0)
				? ApplyCvCal(CvToKnob(CVVal(pattern.cvStepRoute - 1)),
