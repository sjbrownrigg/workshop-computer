...
public:
	...
	*/
	...
	// Call from the OTHER core to fully pause this core's audio/panel
	// interrupt servicing (DMA_IRQ_0 and PWM_IRQ_WRAP) - e.g. for the
	// duration of a flash erase/program. ProcessSample is a virtual call,
	// dispatched through a vtable that (like any const/rodata) lives in
	// flash; merely making ProcessSample's own code RAM-resident does not
	// make that vtable read safe during a flash write. Actually disabling
	// the IRQs (so BufferFull/AudioCallback never run at all, vtable lookup
	// included) is the only way to make this fully safe. Ported verbatim
	// from EvoSeq's ComputerCard.h (~/projects/evo_sequencer/) - proven
	// working in production; see RP2040_dual_core_flash_safety_notes.md.
	//
	// This is a hand-rolled equivalent of multicore_lockout, using plain
	// polling instead of the SDK's SIO-FIFO-based mechanism - the latter
	// was found to break USB enumeration entirely on this hardware/SDK
	// combination. irq_set_enabled() must be called from the core that
	// owns the NVIC entry, so the actual disable happens inside
	// AudioWorker()'s loop on this core, in response to the request below.
	void RequestPause()
	{
		pauseRequested = true;
		while (!paused) { tight_loop_contents(); }
	}
...
private:
	...
	// See RequestPause()/ReleasePause() above.
...
void __not_in_flash_func(ComputerCard::AudioWorker)()
{
	...
	while (1)
	{
		...
		// See RequestPause()/ReleasePause(). Disabling these IRQs (rather
		// than just hoping the handler code is RAM-resident) is what
		// actually stops BufferFull/AudioCallback from running at all -
		// including the virtual ProcessSample() dispatch, which reads a
		// vtable that lives in flash regardless of where ProcessSample's
		// own code is placed. Ported verbatim from EvoSeq's ComputerCard.h.
		if (pauseRequested)
		{
